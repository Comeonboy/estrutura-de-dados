...
def gerar_alfa(s):
    ...
    Tempo = O(n^2), porque cada elemento percorre um FOR dentro de um WHILE
    Espaço = O(3^n), porque cada caracter de entrada vira 3 caracteres de saída
    ...
    for car in s:
        tamanho = len(resposta)-1
        
        while tamanho >= 0:
            item = resposta.pop(tamanho)
            itemR = regra.__getitem__(car)
            
            for x in itemR:
                resposta.append(item + tuple(x,))
